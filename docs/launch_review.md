...
- **⚠️ Action required:** the rules change only takes effect once deployed to Firebase — `firebase deploy --only firestore:rules`, or paste `firestore.rules` into Firebase Console → Firestore → Rules → Publish. **Until then the bypass is live in production.** This deploy could not be done from this environment (no Firebase CLI/credentials).
...
- **Launch blockers (6): all 6 fixed.** MKO badge (§1.1), cross-user session leak (§1.2), old-prod URL fallback (§1.3), dependency pinning (§1.4), missing `/health` (§1.5), and the free-Pro self-grant found in the second pass (§1.6).
  **⚠️ §1.6's fix requires a manual Firebase rules deploy to take effect — until then that bypass is still live in production.**
...
### Testing caveat
Everything above was verified by static review, `python -m py_compile` across all modules, the four standalone test suites, and targeted direct-invocation tests of the new validation and checkout-auth logic (documented inline per finding). There is **no live Firestore or browser environment in this workflow**, so the paths that genuinely need a human check before you trust them are:
1. **The Firestore rules change (§1.6)** — deploy it, then confirm from the browser console that a signed-in user can no longer write `is_pro`, *and* that a brand-new user's first-visit doc still gets created (the `create` rule allows `is_pro: false` specifically to preserve that).
2. **The Stripe checkout flow (§2.1)** — the frontend now sends a bearer token; worth one real end-to-end upgrade attempt.
3. **The import → export flow (§1.2)** — two browser sessions, different imports, confirm exports don't cross over.
